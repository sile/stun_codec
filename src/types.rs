...
use bytecodec::{self, ByteCount, Decode, DecodeExt, Encode, EncodeExt, Eos, Result, SizedEncode};
use std::fmt;
...
impl SocketAddrValue {
    ...
    pub fn encoder() -> impl SizedEncode<Item = Self> {
...
// TODO
pub struct SocketAddrDecoder(Box<Decode<Item = SocketAddr> + Send + 'static>);
...
impl Default for SocketAddrDecoder {
    fn default() -> Self {
        SocketAddrDecoder(Box::new(SocketAddrValue::decoder().map(|v| v.0)))
    }
}
impl fmt::Debug for SocketAddrDecoder {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "SocketAddrDecoder(_)")
    }
}

// TODO
pub struct SocketAddrEncoder(Box<SizedEncode<Item = SocketAddr> + Send + 'static>);
...
impl Default for SocketAddrEncoder {
    fn default() -> Self {
        SocketAddrEncoder(Box::new(
            SocketAddrValue::encoder().map_from(|v| SocketAddrValue(v)),
        ))
    }
}
impl fmt::Debug for SocketAddrEncoder {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "SocketAddrEncoder(_)")
    }
}
